// bruteforce
// TODO: use max-flow to only walk over the ranges
#include <bits/stdc++.h>
using namespace std;
...
int main() {
    ...
    for(int i = 0; i < seeds.size(); i += 2) {
        ...
        for(long long s = seed; s < seed + range; ++s) {
            long long curr_seed = s;
            for(auto &map : maps) {
                curr_seed = convertToMap(map, curr_seed);
            }
            min_location = min(min_location, curr_seed);
